...
### 1.7 The Nexus Integration (Invariant Logic)

**The Nexus is the enforcement engine. It is NOT optional.**
It bridges the gap between "defined protocols" (Text) and "executed behavior" (Code).
...
### 4.1 Turn Counter Checkpoints (AUTOMATIC)
...
**Action**:
```bash
archaiforge query outcomes --limit 1
```
...
Combine the turn counter checkpoint (4.1) with the memory query:
1. Check signature compliance
2. Query ledger
3. Use query result to refresh identity
4. Proceed with strengthened context

### 4.3 Fleur-de-Lis Invariant
